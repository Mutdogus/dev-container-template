...
import { TestLogger, TestLoggerFactory } from './logger';
...
export class DiagnosticSystem {
  ...
  public logTestEnd(
    ...
    this.outputDiagnostic(diagnostic);
    this.testLogger.logTestEnd(testName, passed ? 'passed' : 'failed', duration, metrics);
  }
...
/**
 * Diagnostic utility functions for test operations
 */
export class DiagnosticUtils {
  /**
   * Log test start with structured information
   */
  public static logTestStart(testName: string, context?: Record<string, any>): void {
    const diagnostic: DiagnosticInfo = {
      level: 'info',
      message: `Starting test: ${testName}`,
      timestamp: new Date(),
  ...
  public static logTestEnd(
    ...
    const diagnostic: DiagnosticInfo = {
      ...
      timestamp: new Date(),
      source: 'test-runner',
      details: { duration, passed, ...metrics },
    };

    console.log(`[TEST END] ${testName} - ${status} (${duration}ms)`);
  }

  /**
   * Log container operation with status and details
   */
  public static logContainerAction(
    action: string,
    containerId: string,
    details?: Record<string, any>
  ): void {
    const diagnostic: DiagnosticInfo = {
      level: 'info',
      message: `Container ${action}: ${containerId}`,
      timestamp: new Date(),
      source: 'container',
      details,
    };

    console.log(`[CONTAINER] ${action.toUpperCase()} - ${containerId}`);
  }

  /**
   * Log extension operation with comprehensive details
   */
  public static logExtensionAction(
    action: string,
    extensionId: string,
    extensionName: string,
    details?: Record<string, any>
    ...
    const diagnostic: DiagnosticInfo = {
      level: 'info',
      message: `Extension ${action}: ${extensionName} (${extensionId})`,
      timestamp: new Date(),
      source: 'extension',
      details: { extensionId, extensionName, ...details },
    };

    console.log(`[EXTENSION] ${action.toUpperCase()} - ${extensionName} (${extensionId})`);
  }

  /**
   * Log performance metrics with context
   */
  public static logPerformanceMetric(
    metric: string,
    value: number,
    unit: string,
    context?: Record<string, any>
    ...
    console.log(`[PERFORMANCE] ${metric.toUpperCase()} - ${value}${unit}`);
  }
  ...
  public static logError(
    ...
    const diagnostic: DiagnosticInfo = {
      ...
      timestamp: new Date(),
      source: context,
      details: { error: error.message, stack: error.stack, ...additionalInfo },
    };

    console.error(`[ERROR] ${context.toUpperCase()} - ${error.message}`);
  }
}

// Re-export DiagnosticLogger for test access
export { DiagnosticLogger } from './diagnostic-logger';
