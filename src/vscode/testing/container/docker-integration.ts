...
export class DockerIntegration {
  ...
  constructor(dockerOptions?: Docker.DockerOptions) {
    ...
    this.docker = new (Docker as any)(options as any);
  ...
  public async startContainer(
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      this.logger.error(`Failed to start container: ${errorMessage}`, 'docker-integration', {
        containerId,
      });
      throw error;
    }
  ...
  public async stopContainer(
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      this.logger.error(`Failed to stop container: ${errorMessage}`, 'docker-integration', {
        containerId,
      });
      throw error;
    }
  ...
  public async restartContainer(
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      this.logger.error(`Failed to restart container: ${errorMessage}`, 'docker-integration', {
        containerId,
      });
      throw error;
    }
  ...
  public async removeContainer(
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      this.logger.error(`Failed to remove container: ${errorMessage}`, 'docker-integration', {
        containerId,
      });
      throw error;
    }
  ...
  public async getContainerLogs(
    ...
    try {
      ...
      // Note: since and until may not be supported in this dockerode version
      // if (options.since) {
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      this.logger.error(`Failed to get container logs: ${errorMessage}`, 'docker-integration', {
        containerId,
      });
      throw error;
    }
  ...
  public async getContainerStats(containerId: string): Promise<any> {
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      this.logger.error(`Failed to get container stats: ${errorMessage}`, 'docker-integration', {
        containerId,
      });
      throw error;
    }
    ...
    return new Promise((resolve, reject) => {
      ...
      const checkReady = async () => {
        try {
          ...
          const errorMessage = error instanceof Error ? error.message : String(error);
          this.logger.error(
            `Error checking container readiness: ${errorMessage}`,
            'docker-integration'
          );
          reject(error);
        }
  ...
  public async getContainerResourceUsage(containerId: string): Promise<ResourceUsage | null> {
    try {
      ...
      const cpuPercentage =
        systemDelta > 0 ? (cpuDelta / systemDelta) * (cpuStats.online_cpus || 1) * 100 : 0;
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      this.logger.error(
        `Failed to get container resource usage: ${errorMessage}`,
        'docker-integration',
        { containerId }
      );
      return null;
  ...
  public async pullImage(
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      this.logger.error(`Failed to pull image: ${errorMessage}`, 'docker-integration', {
        imageName,
      });
      throw error;
    }
