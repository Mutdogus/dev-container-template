      ...
      const testResult = await testRunner.runTestWithRetry(
        ...
        async () => {
          ...
          const missingExtensions = expectedExtensions.filter(
            expectedId => !extensionStatuses.some(status => status.id === expectedId)
          );
          ...
          const slowExtensions = extensionStatuses.filter(
            ext => ext.loadTime && ext.loadTime > 30000 // 30 seconds
          );
    ...
    it('should detect extension loading failures', async () => {
      ...
      const testResult = await testRunner.runTestWithRetry(
        ...
        async () => {
          ...
          const criticalFailures = extensionStatuses.filter(
            status =>
              status.status === 'failed' &&
              ['ms-vscode.cpptools', 'github.copilot'].includes(status.id)
          );
        ...
        async () => {
          const compatibilityResults = extensions.map(ext => {
            ...
            return {
              ...
              issues:
                isCompatible && maxVersionCompatible
                  ? []
                  : [`VS Code version ${vscodeVersion} incompatible with extension requirements`],
            };
          });

          const incompatibleExtensions = compatibilityResults.filter(
    ...
    it('should validate extension loading performance', async () => {
      ...
      const testResult = await testRunner.runTestWithRetry(
        ...
        async () => {
          ...
          const totalLoadingTime = extensionStatuses.reduce(
            (sum, ext) => sum + (ext.loadTime || 0),
            0
          );
          const slowExtensions = extensionStatuses.filter(
            ext => ext.loadTime && ext.loadTime > maxLoadingTime
          );
    ...
    it('should measure extension memory usage', async () => {
      ...
      const testResult = await testRunner.runTestWithRetry(
        ...
        async () => {
          ...
          const highMemoryExtensions = extensionMemoryUsage.filter(
            ext => ext.usage > memoryThreshold
          );
